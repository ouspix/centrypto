// Test devstral specifically
const testDevstral = async () => {
    const snapshot = {
        account: {
            equity: 10000,
            positions: []
        ...
        markets: {
            'BTC-PERP': {
                ...
                volume: 1000000,
                orderbook: {
                    bids: [[49950, 10], [49900, 20]],
                    asks: [[50050, 10], [50100, 20]]
                }
            }
        },
        sentiment: 0.65
...
{"action":"DO_NOTHING","symbol":null,"side":null,"size_fraction_of_equity":null,"risk_plan":null,"playbook":"none","confidence":0.5,"reason_code":"no_trade","notes":"Insufficient data to make a decision."}`;
    ...
    try {
        console.log('🧪 Testing devstral:latest\n');
        ...
        try {
            ...
            if (jsonStr.includes('```')) {
                console.log('   Found markdown code blocks');
                ...
                if (match) {
                    ...
                    if (firstBrace !== -1 && lastBrace !== -1) {
                        ...
                        console.log('   Extracted between braces');
                    }
                }
            }
            ...
            console.log('   Reason:', parsed.reason_code);
